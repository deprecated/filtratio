...
** STARTED How to deal with composite filters
:LOGBOOK:  
CLOCK: [2014-11-10 Mon 10:30]
...
+ Initial plan is to subclass Bandpass with CompositeBandpass
  ...
  + The =Ti= method is easy to deal with: just add them up
    + But not clear that this is the best approach, although it does deal with the ad hoc adjustments automatically
  + Leave it for the moment, but we also need to separately calculate other properties: wave, T, T, Wj, wav0
